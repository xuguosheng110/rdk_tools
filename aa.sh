...
BACK_UP_DIR=$BACKUP_DIR/rdk-backup
FILE=$BACK_UP_DIR/rdk-backup.img  #备份后的img文件名
mkdir $BACK_UP_DIR

#创建镜像img文件
echo -e "$Color_Green创建img文件...$Color_End"
ROOT=`df -P | grep /dev/root | awk '{print $3}'`   #获取 ROOT的文件大小
MMCBLK0P1=`df -P | grep /dev/mmcblk2p1 | awk '{print $2}'`  #获取主目录的文件大小
...
P1_START=`fdisk -l /dev/mmcblk2 | grep /dev/mmcblk2p1 | awk '{print $2}'`
P1_END=`fdisk -l /dev/mmcblk2 | grep /dev/mmcblk2p1 | awk '{print $3}'`
P2_START=`fdisk -l /dev/mmcblk2 | grep /dev/mmcblk2p2 | awk '{print $4}'`
